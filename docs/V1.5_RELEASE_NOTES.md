# 📊 Scanner Binance - Release Notes V1.5

## Version 1.5 - Moyennes Mobiles Multi-Timeframe

**Date de release** : 17 janvier 2026  
**Status** : ✅ **COMPLÈTE ET VALIDÉE**

---

## 🎯 Objectifs atteints

Cette version ajoute l'analyse des **moyennes mobiles** (SMA et EMA) sur **3 timeframes** (Hebdo, Daily, H4) pour détecter les tendances haussières et filtrer les opportunités de trading de manière plus précise.

### Nouvelles fonctionnalités

#### 1. Calcul des Moyennes Mobiles (indicators.py)
- ✅ **SMA (Simple Moving Average)** : Moyenne arithmétique sur N périodes
- ✅ **EMA (Exponential Moving Average)** : Moyenne exponentielle avec poids décroissant
- ✅ Périodes configurables : 20 et 50 (par défaut)
- ✅ Gestion robuste des cas limites (données insuffisantes, NaN)

#### 2. Détection de Tendance Multi-Timeframe
- ✅ **Logique de détection** : 
  - Haussière si : (Prix > SMA20 ET Prix > SMA50) OU (EMA20 > EMA50)
  - Baissière sinon
- ✅ **Analyse sur 3 timeframes** :
  - **1w (Hebdomadaire)** : Tendance long terme
  - **1d (Daily)** : Tendance moyen terme  
  - **4h** : Tendance court terme
- ✅ **Trend Score (0-3)** : Nombre de timeframes haussiers

#### 3. Filtre Combiné RSI + Tendance
- ✅ Filtrage strict : `RSI < threshold` **ET** `trend_score >= MIN_TREND_SCORE`
- ✅ Évite les faux signaux (RSI bas sans tendance haussière)
- ✅ Identifie les vraies opportunités (RSI bas + rebond probable)

#### 4. Output Enrichi
**Console** :
- Affichage du trend_score pour chaque paire
- Flags visuels pour chaque timeframe (✓ haussier / ✗ baissier)
- Tableau élargi avec colonnes supplémentaires

**CSV** :
- 12 colonnes de moyennes mobiles :
  - `sma20_1w`, `sma50_1w`, `ema20_1w`, `ema50_1w`
  - `sma20_1d`, `sma50_1d`, `ema20_1d`, `ema50_1d`
  - `sma20_4h`, `sma50_4h`, `ema20_4h`, `ema50_4h`
- 3 colonnes de flags de tendance :
  - `trend_1w` (bool)
  - `trend_1d` (bool)
  - `trend_4h` (bool)
- 1 colonne de score global :
  - `trend_score` (0-3)

---

## ⚙️ Configuration (config.py)

### Nouveaux paramètres V1.5

```python
# MOYENNES MOBILES (V1.5)
ENABLE_MA = True                      # Activer/désactiver l'analyse MA
MA_PERIODS = [20, 50]                 # Périodes à calculer
MA_TIMEFRAMES = ["1w", "1d", "4h"]   # Timeframes à analyser
MIN_TREND_SCORE = 2                   # Score minimum requis (0-3)
MIN_MA_BARS = 60                      # Nombre de bougies pour SMA50
```

### Paramètres optimaux recommandés

**Pour détecter les opportunités de qualité** :
- `RSI_THRESHOLD = 35` : Zone de survente
- `MIN_TREND_SCORE = 2` : Au moins 2 timeframes haussiers sur 3

**Pour scanner plus largement** :
- `RSI_THRESHOLD = 40-45` : Zone de faiblesse
- `MIN_TREND_SCORE = 1` : Au moins 1 timeframe haussier

**Pour être très sélectif** :
- `RSI_THRESHOLD = 30` : Survente forte
- `MIN_TREND_SCORE = 3` : Les 3 timeframes doivent être haussiers

---

## 📊 Exemple de Résultat

```
========================================================================================================================
RÉSULTATS DU SCAN - RSI < 35
TENDANCE HAUSSIÈRE ≥ 2/3 timeframes
========================================================================================================================
 Symbole   RSI           Prix             Date TF  Trend 1W 1D 4H
XRP/USDC 32.15     2.05400000 2026-01-17 16:00 4h      2  ✗  ✓  ✓
BTC/USDC 34.82 95000.00000000 2026-01-17 16:00 4h      3  ✓  ✓  ✓
========================================================================================================================
Total: 2 paire(s)
========================================================================================================================
```

**Interprétation** :
- **XRP/USDC** : RSI faible (32.15) + tendance haussière sur Daily et H4 → Opportunité potentielle
- **BTC/USDC** : RSI faible (34.82) + tendance haussière sur TOUS les timeframes → Opportunité forte

---

## 🧪 Tests

### Tests unitaires (test_modules.py)
```
✓ PASS - Indicators
  ✓ RSI calculé correctement
  ✓ SMA cohérente (SMA20 > SMA50 sur données croissantes)
  ✓ EMA cohérente (EMA20 > EMA50 sur données croissantes)
  ✓ Détection de tendance correcte (haussière sur données croissantes)
```

### Test d'intégration
```bash
python main.py
# Configuration : RSI < 60, MIN_TREND_SCORE = 1, MAX_PAIRS = 5

Résultat : 4/5 paires trouvées
- XRP/USDC : RSI=48.44 | Trend=1/3
- BTC/USDC : RSI=55.07 | Trend=2/3
- XLM/USDC : RSI=55.98 | Trend=2/3
- ETH/USDC : RSI=58.53 | Trend=3/3

CSV exporté avec succès : 24 colonnes (base + 12 MA + 3 flags + trend_score + métadonnées)
```

---

## 📈 Performance

### Temps d'exécution (estimé)
- **5 paires** : ~5 secondes
- **50 paires** : ~50 secondes (rate limit respecté)
- **500 paires** : ~8-10 minutes
- **Toutes les paires USDC (~600)** : ~10-15 minutes

### Optimisations possibles (futures versions)
- Cache des données OHLCV
- Parallélisation des appels API (avec précaution sur les rate limits)
- Pré-filtrage par volume pour réduire le nombre de paires scannées

---

## 🔄 Migration depuis V1

### Changements **non breaking**
- Tous les anciens modules fonctionnent sans modification
- L'analyse MA peut être désactivée avec `ENABLE_MA = False`
- Si désactivée, le scanner fonctionne exactement comme en V1

### Modules modifiés
1. **config.py** : 5 nouveaux paramètres ajoutés
2. **indicators.py** : 3 nouvelles fonctions (calculate_sma, calculate_ema, detect_trend)
3. **scanner.py** : 
   - Nouvelle fonction `analyze_pair_ma()`
   - Logique de filtrage enrichie dans `scan_market()`
4. **output.py** :
   - Affichage console élargi avec trend_score et flags
   - Export CSV avec 12 colonnes MA supplémentaires
5. **test_modules.py** : Tests enrichis pour SMA/EMA/detect_trend

---

## ✅ Checklist de validation V1.5

- [x] calculate_sma() implémenté et testé
- [x] calculate_ema() implémenté et testé
- [x] detect_trend() implémenté et testé
- [x] Paramètres MA ajoutés à config.py
- [x] Multi-timeframe (1w, 1d, 4h) fonctionnel
- [x] Trend_score calculé correctement
- [x] Filtre combiné RSI + tendance opérationnel
- [x] Affichage console enrichi
- [x] Export CSV avec toutes les colonnes MA
- [x] Tests unitaires passent (6/6)
- [x] Test d'intégration validé
- [x] Performance acceptable (< 15 min pour toutes les paires)
- [x] Documentation mise à jour

---

## 🚀 Prochaines étapes (V2.0 potentielle)

**Non planifié actuellement** - Le projet reste un scanner uniquement.

Évolutions possibles **si demandées** :
- Volume analysis (filtrer par volume 24h minimum)
- Bollinger Bands ou autres indicateurs
- Alertes (Discord, Telegram, email)
- Interface web/dashboard
- Historique des scans

---

## 📝 Notes importantes

### ⚠️ Limitations connues
- Le scanner peut être lent sur un grand nombre de paires (rate limits Binance)
- Nécessite une connexion internet stable
- Les données de certaines paires peuvent être incomplètes

### 🔒 Règles strictes maintenues
- **Aucun trading** : Ce projet reste un scanner uniquement
- **Lecture seule** : Aucune modification de compte
- **Données publiques uniquement** : Pas de clés API nécessaires

---

## 👥 Contribution

Pour toute suggestion ou amélioration :
1. Respecter le cahier des charges (scanner uniquement)
2. Suivre la structure modulaire existante
3. Ajouter des tests pour toute nouvelle fonctionnalité
4. Documenter les changements

---

**Version** : 1.5  
**Status** : Production Ready ✅  
**Dernière mise à jour** : 17 janvier 2026
